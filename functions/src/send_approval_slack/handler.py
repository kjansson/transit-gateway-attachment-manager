...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Lambda function to send approval message to Slack for Step Functions human approval task.

    This function extracts execution context and API Gateway endpoint from the event,
    then constructs approve/reject URLs and posts an approval message to Slack.
    ...
    Returns:
        Dict containing the message, subject, and approval URLs
    """
    ...
    try:
        ...
        # Construct message (same format as approval email, adapted for Slack)
        message = (
            "Welcome!\n\n"
            "This is a message requiring an approval for a step functions execution.\n\n"
        ...
        logger.info('Approval message generated successfully')
        ...
        if slack_webhook_url:
            try:
                ...
                with urlopen(req) as resp:
                    slack_response = resp.read().decode('utf-8')
                logger.info(f'Message posted to Slack. Response: {slack_response}')
                response['slackResponse'] = slack_response
            except (HTTPError, URLError) as e:
                logger.error(f'Failed to post to Slack: {str(e)}')
                response['slackError'] = str(e)
        else:
            logger.info('Slack webhook URL not provided, skipping Slack post')
